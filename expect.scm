(define (expect-avatar-set! glyph) (set! avatar glyph))
(define avatar #f)
...
(define (expect-generic res tries)
  (and (not (string=? res ""))
	   ...
	   (and avatar			; not invisible
		(equal? (square-glyph (get-coord)) avatar)
		(botl-update)
		;; often a delay before topl gets printed, so extra
		;; read if topl is empty
