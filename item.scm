...
(define (item-named item) (get-submatch item 'named #f))
...
(define (item-identity item)
  (cond
   ...
   ((get-item-field item 0) => identity)
   ; common cases:
   ((and (tool? item)
...
(define (item-appearance item)
  (let ((name (item-name item)))
    (cond
     ...
     ((get-item-field item 6) => identity)
     ((and (item-identity item)
	   (string=? (item-identity item) "potion of water"))
      "clear potion")
     ((unidentified-gem? (item-name item)) (item-name item))
